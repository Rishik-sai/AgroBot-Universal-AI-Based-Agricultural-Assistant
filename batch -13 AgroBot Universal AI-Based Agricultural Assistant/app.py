...
# 5. Gemini AI setup
# -------------------------------------------------------------------
# Model order matters: first one wins, the rest are fallbacks for 404/429.
# gemini-2.5-* were retired for new API keys, so they are not defaults.
GEMINI_MODELS = [
...
def test_gemini():
    try:
        ...
        # Walk the same fallback chain the chat path uses, so a transient 503
        # on the preferred model does not report the whole API as broken.
        attempts = []
        for model_name in GEMINI_MODELS:
            try:
                ...
                return jsonify({
                    ...
                    "model": model_name,
                    "fell_back": model_name != GEMINI_MODELS[0],
                    "attempts": attempts,
...
def test_key():
    return jsonify({
        "weather_api_key_configured": bool(app.config.get('WEATHER_API_KEY')),
    })
...
def init_db():
    try:
        ...
        if not admin_user:
            ...
            admin_user.set_password(_seed_password('ADMIN_PASSWORD', 'admin'))
            ...
            demo_user.set_password(_seed_password('DEMO_PASSWORD', 'demo'))
...
def _seed_password(env_var, label):
    ...
    Taken from the environment so no credential lives in source. With the var
    unset we mint a random one and print it once -- the account stays usable
    without ever shipping a known default.
    """
    ...
    print(f"🔐 Generated {label} password (set {env_var} to choose your own): {pw}")
    return pw
    ...
    with app.app_context():
        try:
            ...
            if not admin_exists:
                ...
                admin.set_password(_seed_password('ADMIN_PASSWORD', 'admin'))
                ...
                demo.set_password(_seed_password('DEMO_PASSWORD', 'demo'))
                db.session.add(demo)
                db.session.commit()
...
if __name__ == "__main__":
    ...
    print("🔑 Admin account: admin@aiagrobot.com")
    print("🔑 Demo account:  demo@aiagrobot.com")
    print("   (passwords come from ADMIN_PASSWORD / DEMO_PASSWORD, else generated above)")
